...
qnorm(c(0.05,0.25,0.5,0.75,0.95),0,1,lower.tail = TRUE)
plot(x,pnorm(x,mean = 0,sd = 1),type = "l",col = "blue",ylab = "probability",
     xlab = "standard normal variable",main = "Standard Normal CDF Function")
abline(h = c(0.05,0.25,0.5,0.75,0.95),lty = 2,col = "green")
abline(v = qnorm(c(0.05,0.25,0.5,0.75,0.95),0,1,lower.tail = TRUE),col = "red")
?abline()
...
cord.x <- c(-3,seq(-3,0.67,0.1),0.67)
cord.y <- c(0,dnorm(seq(-3,0.67,0.1),0,1),0)
...
quant_w <- quantile(normal_x, probs = c(0.01,0.05,0.25,0.5,0.75,0.95,0.99),type=7)
...
points(x,dbinom(x,n,p),pch = 16,cex = 1)
curve(dnorm(x,mean = n*p,sd = sqrt(n*p*(1-p))),add = TRUE,col = "orange",lwd = 2)
legend("topright",legend = c("binomial probability in blue","normal density in orange",
                             "p = 0.1, n = 25"))
...
x <- seq(0,5,0.1)
plot(x,dexp(x,r = 1),type = "l",col = "red",ylab = "density",main = 
...
for (i in 1:N)
{
phat <- sum(rbinom(1, size = n, prob = p))/n
